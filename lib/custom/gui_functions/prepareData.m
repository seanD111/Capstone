...
for i=1:length(uniqueSelectedClips)
    ...
    for h=1:length(allLangs)
        full_languageTargets(h,i)=strcmp(uniqueSelectedLangs{1,i}, allLangs{1,h});          
...
%         windowed_languageInputs(windowed_rowInd+numRows, :)=tempFormPow;
    ...
    for h=1:length(allLangs)
        ...
        windowed_languageTargets(h,windowed_colEnd:(windowed_colEnd+numFrames-1))=strcmp(uniqueSelectedLangs{1,i}, allLangs{1,h});          
